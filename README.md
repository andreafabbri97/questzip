...
- [x] 🇮🇹 Compendio in italiano dai manuali ufficiali — **chiuso** (vedi `scripts/ita-compendio/`): incantesimi (Manuale del Giocatore, Tasha, Xanathar), mostri (6 manuali, filtrati voce per voce incrociando i dati con l'inglese), razze (le 9 del Manuale del Giocatore, sottorazze incluse) e classi (dado vita/competenze/equipaggiamento e tabella di progressione livelli **complete per tutte e 12**, incluse le 4 classi incantatrici principali che inizialmente sembravano irrecuperabili — l'estrazione geometrica non si affida più al numero di livello stampato, spesso assente dal testo, ma conta le righe della tabella con il bonus di competenza come ancora principale). Nuova sezione "📚 Regole" nel Compendio con "Regole Principali" e "Guida agli Avventurieri della Costa della Spada" (entrambe scansioni pure senza testo digitale, estratte via OCR — badge esplicito sulla qualità nel testo). Fuori scope confermato: oggetti magici (il PHB non ne contiene, solo il DMG), "Oggetti Magici (Dm e Book of many things).pdf" (risultato in inglese via OCR). **Manuale del DM**: diagnosi iniziale (font offuscato, ~2100 sotto-insiemi senza mappa Unicode) sembrava bloccante, poi trovato un approccio che funziona — OCR dell'intera pagina renderizzata (leggibile normalmente) allineato per posizione col testo geometricamente corretto ma semanticamente sbagliato di PyMuPDF, per ricostruire un dizionario "codice cifrato → lettera vera" per ogni sotto-insieme di font — in corso.
