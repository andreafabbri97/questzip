...
- **🛡️ Personaggi** — razza/classe con autocompletamento dal Compendio, multiclasse (più classi, con sottoclasse e descrizione "come funziona" per ciascuna), generatore caratteristiche (manuale, array standard, acquisto punti, tiro dadi), calcolatore PF multiclasse (1° livello della classe di origine massimizzato, il resto in media), punti ferita (contatore rapido +/−), CA, velocità, bonus di competenza, note libere, export/import in JSON. Se sei loggato puoi portare il personaggio in una campagna condivisa (snapshot aggiornabile a mano, non sync live).
